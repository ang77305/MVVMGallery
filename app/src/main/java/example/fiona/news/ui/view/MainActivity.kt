...
class MainActivity :
    ...
    private val adapterList = mutableListOf<PixabayHitsData>()
                ...
                if (recyclerView.layoutManager is LinearLayoutManager) {
                    ...
                    if (!isLoading) {
                        if (linearLayoutManager != null &&
                            linearLayoutManager.findLastCompletelyVisibleItemPosition() == recyclerView.adapter?.itemCount?.minus(
                                5
                            )
                        ) {
                    ...
                    if (!isLoading) {
                        if (gridLayoutManager != null &&
                            gridLayoutManager.findLastCompletelyVisibleItemPosition() == recyclerView.adapter?.itemCount?.minus(
                                5
                            )
                        ) {
                            mainViewModel.loadMore(search_view.query.toString())
